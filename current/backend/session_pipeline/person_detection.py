...
from .config import (
    MODEL_PATH, MODEL_INPUT_SIZE, PERSON_CLASS_ID,
    ...
    def run(self):
        while not self.stop_event.is_set():
            ...
            if self.person_seen >= START_CONFIRM_FRAMES:
                event_queue.put_nowait({"type": "START_RECORDING", "timestamp": item["timestamp"]})
                self.person_seen = 0

            if self.person_missing >= STOP_CONFIRM_FRAMES:
                event_queue.put_nowait({"type": "STOP_RECORDING", "timestamp": item["timestamp"]})
                self.person_missing = 0
